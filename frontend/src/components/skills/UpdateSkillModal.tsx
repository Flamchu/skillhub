...
export function UpdateSkillModal({ isOpen, onClose, skill, onUpdateSkill, onRemoveSkill }: UpdateSkillModalProps) {
	const t = useTranslations("skills.updateModal");
	...
	const proficiencyOptions: Array<{
		...
		{
			value: "BASIC",
			label: tCommon("proficiency.basic"),
			description: t("levels.basic"),
			percentage: 25,
		...
		{
			value: "INTERMEDIATE",
			label: tCommon("proficiency.intermediate"),
			description: t("levels.intermediate"),
			percentage: 50,
		...
		{
			value: "ADVANCED",
			label: tCommon("proficiency.advanced"),
			description: t("levels.advanced"),
			percentage: 75,
			icon: <TrendingUp className="h-4 w-4" />,
			color: "text-primary",
		},
		{
			value: "EXPERT",
			label: tCommon("proficiency.expert"),
			description: t("levels.expert"),
			percentage: 90,
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
			...
			<div className="flex items-center justify-between mb-6">
				<div>
					...
					<p className="text-sm text-gray-600 dark:text-gray-400">{t("description", { skill: skill.skill.name })}</p>
				</div>
					<button
						onClick={onClose}
				...
				{/* Current Level */}
				<div className="mb-6">
					<h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t("currentLevel")}</h3>
					<div className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
						...
						<div>
							...
							<div className="text-sm text-gray-600 dark:text-gray-400">
								{currentProficiency?.description || t("noDescription")}
							</div>
						</div>
					</div>
				</div>
				...
				{/* New Level Selection */}
				<div className="mb-6">
					<h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{t("selectLevel")}</h3>
				...
				<div className="space-y-3">
							...
							{!showRemoveConfirm ? (
								...
								<div className="space-y-3">
									...
									<div className="flex items-center space-x-3">
										...
										>
											{removing ? t("remove.removing") : t("remove.confirmButton")}
										</Button>
									</div>
								</div>
							)}
					...
					{!showRemoveConfirm && (
						<div className="flex items-center space-x-3">
							...
							>
								{updating ? t("actions.updating") : isUpgrading() ? t("actions.updateAndVerify") : t("actions.update")}
							</Button>
						</div>
					)}
				</div>
