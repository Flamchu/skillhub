			...
			{quickActions.map(action => {
				...
				return (
					<Link key={action.href} href={action.href}>
						<div
							className={`group ${colors.bg} rounded-xl p-6 border border-gray-200/50 dark:border-gray-700/50 ${colors.hover} transition-all duration-200 hover:shadow-lg hover:scale-105`}
						>
							<div className="flex items-start space-x-4">
								<div
									className={`shrink-0 w-12 h-12 ${colors.icon} rounded-lg flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform`}
								>
									{action.icon}
								</div>
