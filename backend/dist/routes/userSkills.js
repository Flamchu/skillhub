...
router.get("/:userId/skills", async (req, res) => {
    try {
        ...
        if (includeProgress === "true") {
            enrichedSkills = await Promise.all(skills.map(async (userSkill) => {
...
router.post("/:userId/skills", auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        const validProficiencyLevels = Object.values(client_1.ProficiencyLevel);
...
router.patch("/:userId/skills/:skillId", auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (lastPracticed !== undefined)
            updateData.lastPracticed = new Date(lastPracticed);
...
router.delete("/:userId/skills/:skillId", auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId, skillId } = req.params;
        const currentUser = req.user;
...
router.get("/:userId/skills/:skillId/progression", auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId, skillId } = req.params;
        const currentUser = req.user;
        ...
        const getNextSteps = (proficiency, targetLevel) => {
            const steps = [];
...
router.patch("/:userId/skills", auth_1.authenticateToken, async (req, res) => {
    try {
        const { userId } = req.params;
        const currentUser = req.user;
        // users can only update their own skills unless they're admin
        ...
        const updatePromises = skills.map((skill) => {
            ...
            if (skill.targetLevel !== undefined)
                updateData.targetLevel = skill.targetLevel;
            if (skill.progress !== undefined)
                updateData.progress = skill.progress;
            if (skill.lastPracticed !== undefined)
                updateData.lastPracticed = new Date(skill.lastPracticed);
