...
router.get("/:id", async (req, res) => {
    try {
        ...
        if (includeStats === "true") {
            ...
            const proficiencyDistribution = userSkillStats.reduce((acc, group) => {
...
router.get("/search/advanced", async (req, res) => {
    try {
        ...
        minUsers = "0", maxUsers, hasCourses = "false", hasTests = "false", level, // hierarchy level (0 = root, 1 = first level, etc.)
        limit = "20", offset = "0", } = req.query;
        const where = {};
        // text search
        if (q) {
            where.OR = [{ name: { contains: q, mode: "insensitive" } }, { description: { contains: q, mode: "insensitive" } }, { slug: { contains: q, mode: "insensitive" } }];
        }
        ...
        const maxUsersNum = maxUsers ? parseInt(maxUsers) : undefined;
        filteredSkills = filteredSkills.filter((skill) => {
            const userCount = skill._count.UserSkill;
            if (userCount < minUsersNum)
                return false;
            if (maxUsersNum && userCount > maxUsersNum)
                return false;
            if (hasCourses === "true" && skill._count.courses === 0)
                return false;
            if (hasTests === "true" && skill._count.tests === 0)
                return false;
            return true;
        });
        ...
        res.json({
            ...
            pagination: {
                ...
                hasMore: parseInt(offset) + filteredSkills.length < totalCount,
            },
        });
    }
    catch (error) {
...
router.delete("/:id", auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (skill._count.children > 0)
            dependencies.push(`${skill._count.children} child skills`);
        if (skill._count.UserSkill > 0)
            dependencies.push(`${skill._count.UserSkill} user associations`);
        if (skill._count.courses > 0)
            dependencies.push(`${skill._count.courses} course associations`);
        if (skill._count.tests > 0)
            dependencies.push(`${skill._count.tests} tests`);
